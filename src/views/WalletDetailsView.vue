  ...
  <v-container fluid class="py-8 px-4 bg-background wallet-details-min-height">
    ...
    <v-row v-if="walletStore.activeWallet">
      <v-col cols="12" class="text-center">
        ...
        <p class="text-h5 font-weight-bold" :class="{'text-success': walletStore.activeWallet.saldo >= 0, 'text-error': walletStore.activeWallet.saldo < 0}">
    ...
    <v-row v-if="walletStore.activeWallet" class="mt-8 justify-center">
      <v-col cols="12" md="8"> <!-- Alterado para ocupar mais espaço se o formulário for removido -->
        <v-card class="pa-6 elevation-2 rounded-lg h-full" color="surface">
          <v-card-title class="text-h6 font-weight-bold text-primary">Membros da Caixinha</v-card-title>
          <v-card-text>
            <v-list class="bg-surface overflow-y-auto" style="max-height: 250px;"> <!-- Altura máxima ajustada -->
              <v-list-item v-for="memberId in walletStore.activeWallet.membros" :key="memberId">
                ...
                <v-list-item-title class="text-high-emphasis">
                    ...
                    <span v-if="memberId === walletStore.activeWallet.idUsuarioPrincipal" class="text-medium-emphasis text-caption">(Principal)</span>
                </v-list-item-title>
              </v-list-item>
            </v-list>
      ...
      <!-- REMOVIDA A COLUNA DO FORMULÁRIO DE TRANSAÇÃO -->
    </v-row>

    <v-row v-if="walletStore.activeWallet" class="mt-8">
      <v-col cols="12">
        <v-card class="pa-6 elevation-2 rounded-lg" color="surface">
          <v-card-title class="text-h6 font-weight-bold text-primary">Todas as Transações</v-card-title>
          <v-card-text>
            <div class="overflow-x-auto">
              ...
              >
                <template v-slot:item.valor="{ item }">
                  <span :class="{'text-success': item.tipo === 'ganho', 'text-error': item.tipo === 'despesa'}" class="font-weight-bold">
            ...
            <p v-if="!transactionStore.transactions.length" class="text-medium-emphasis text-center mt-4">Nenhuma transação registrada para esta caixinha.</p>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>

    <!-- Modal de Convite (mantido) -->
    <v-dialog v-model="showInviteModal" max-width="500px">
      <v-card color="surface">
        <v-toolbar color="primary" flat>
          <v-toolbar-title class="text-h5 text-white">Convidar Membro para a Caixinha</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn icon @click="showInviteModal = false"><v-icon>mdi-close</v-icon></v-btn>
        </v-toolbar>
        <v-card-text class="pa-6">
          <p class="text-medium-emphasis mb-4">Envie este código de convite para quem você deseja adicionar a esta caixinha:</p>
          <v-text-field
          ...
          ></v-text-field>
          <v-alert v-if="inviteMessage" :type="inviteMessageType" class="mt-4">{{ inviteMessage }}</v-alert>
          <p class="text-medium-emphasis mt-4"> (Em um sistema real, o usuário convidado usaria este código para se juntar à caixinha.)</p>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
const newTransaction = ref({ // Mantido para caso de uso futuro ou contexto
  tipo: 'despesa',
  valor: 0,
  descricao: '',
  categoriaNome: '',
...
// addTransaction (removido, agora no App.vue)
